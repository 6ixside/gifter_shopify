...
class GiftBasket < Sinatra::Base
  ...
  get '/gifter/install' do
    ...
    install_url = "http://#{shop}/admin/oauth/authorize?client_id=#{API_KEY}"\
                "&scope=#{scopes}&redirect_uri=https://#{APP_URL}/gifter/auth"

    get_shop_access_token(shop,API_KEY,API_SECRET,code)
    install_script_tag(shop,API_KEY,API_SECRET,code)
  ...
  post '/gifter/webhook/order_create' do
  ...
  get '/test' do
    content = javascript_tag("alert('test');")

    return [200, content]
  end

  get '/proxy/' do
    #install_script_tag(API_KEY,API_SECRET)
  end
  ...
  helpers do
    ...
    def install_script_tag(shop,client_id,client_secret,code)
    ...
    def create_order_webhook
      ...
      unless ShopifyAPI::Webhook.find(:all).any?
        webhook = {
          topic: 'orders/create',
          address: "https://#{APP_URL}/gifter/webhook/order_create",
          format: 'json'}
